...
loop:
	...
	for (bp = this->bFreeList.av_forw; bp != &(this->bFreeList); bp = bp->av_forw)
		if ((bp->b_flags & Buf::B_ISLOG) != Buf::B_ISLOG)
			break;
	if (bp == &(this->bFreeList))
	{
		X86Assembly::STI();

		this->Bflush(DeviceManager::NODEV);
	...
	// if ((bp->b_flags & Buf::B_ISLOG) == 0 && (bp->b_flags & Buf::B_DELWRI))
	// {

	// 	bp->b_flags |= Buf::B_ASYNC;
	// 	LogManager *lm = &Kernel::Instance().GetLogManager();
	// 	lm->WriteLog(bp);
	// 	goto loop;
	// }
...
void BufferManager::Brelse(Buf *bp)
{
	 ...
	 */
	bp->b_flags &= ~(Buf::B_WANTED | Buf::B_BUSY | Buf::B_ASYNC); // brelease的时候，即使出现了ISLOG，也不能把它去掉
...
void BufferManager::Bwrite(Buf *bp)
{
	...
	bp->b_flags &= ~(Buf::B_READ | Buf::B_DONE | Buf::B_ERROR | Buf::B_DELWRI | Buf::B_ISLOG); // 设置为从日志区读来的,此时消除ISLOG
...
loop:
	...
	{
		/* 找出自由队列中所有延迟写的块 */
		if ((bp->b_flags & Buf::B_ISLOG) && (dev == DeviceManager::NODEV || dev == bp->b_dev))
		{
			bp->b_flags &= ~(Buf::B_DONE);
			this->NotAvail(bp);
			// 替换成this->// this->Bwrite(bp);
...
Buf * BufferManager::getmbuf()
{
		return this->m_Buf;
}
unsigned char * BufferManager::getBuffer()
{
